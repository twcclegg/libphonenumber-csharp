...
- **Never fetch the previous release tag with `--depth=1`.** A shallow fetch grafts it as a parentless root and writes `.git/shallow` even into a full clone, after which `v<tag>..HEAD` excludes only the tag commit rather than its ancestry — measured here, one range grew from 350 commits to 1648 and another collapsed to zero, either of which silently corrupts the fold decision. `--is-ancestor` still answers yes in that state and is no guard; `--is-shallow-repository` is. This is why the checkout uses `fetch-depth: 0` (with `filter: blob:none` to keep it cheap) rather than fetching the tag on demand.
