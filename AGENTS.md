...
- `lib/check-target-frameworks.sh` — automation behind `check_target_frameworks.yml`: compares the target frameworks in `csharp/**/*.csproj` against the .NET versions Microsoft still supports and files an issue when they have drifted apart. README.md ("Target framework updates") documents the dry-run and override options.
...
- **Target frameworks.** `check_target_frameworks.yml` runs monthly and files an issue when a new .NET version has been released or one this repo targets has passed end of support (a release candidate does not count as released). The retarget itself is by hand, and these all move in the same change: the `TargetFramework(s)` of every project under `csharp/` — multi-target projects take the whole supported set, single-target ones (tools, demo, benchmarks) the newest — the `global.json` SDK pin, `dotnet-version` in every workflow, the newest TFM wherever a workflow or project file names it literally (`--framework`, `-p:TargetFrameworks=`, the reproducibility check's TFM list, the MetadataBuilder output path), the `RuntimeMoniker` on the BenchmarkDotNet jobs, and the TFM lists and commands quoted in README.md, this file and CONTRIBUTING.md. `netstandard2.0` is not a .NET version and stays: it is what a consumer on a retired runtime resolves once the asset for their own TFM is gone, which is why dropping a TFM is not a breaking change for them. `#if NET*_OR_GREATER` guards name the floor a code path needs rather than a target, so leave them alone and keep guarding by TFM name rather than by .NET version.
