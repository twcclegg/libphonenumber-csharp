...
- `docfx/` — DocFX config for the generated API reference site, deployed alongside the demo (same `deploy-demo.yml` run, under `/docs/`) so they share one Pages deployment. `docfx/build.sh` copies `docs/*.md` in as conceptual articles (rewriting their repo-relative source links to GitHub permalinks) before building — run it rather than invoking `docfx` directly. `docfx/template/public/main.css` ports the demo's design tokens (`csharp/PhoneNumbers.Demo/wwwroot/css/app.css`) onto DocFX's `modern` template, light and dark, so the two sites match. `docs_preview.yml` builds it on every PR and uploads the static site as a downloadable Actions artifact, so a reviewer can see the rendered result without merging or publishing to Pages.
- `assets/brand/` — the logomark and favicon SVGs, shared by the Blazor demo (copied into its `wwwroot/` by a `Content`/`Link` item in `PhoneNumbers.Demo.csproj`) and the docfx site (mapped to `images/` by `docfx/docfx.json`). One copy so the two sites can't drift.
