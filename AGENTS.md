...
- `lib/update-target-frameworks.sh` — automation behind `update_target_frameworks.yml`: works out which .NET versions Microsoft still supports and retargets every csproj, `global.json`, workflow and doc that names a target framework at them. README.md ("Target framework updates") documents the dry-run and override options.
...
- **Target frameworks are automation's, not yours to guess.** `update_target_frameworks.yml` runs every December 1 (a fortnight after .NET's November release) and opens a PR retargeting the repository at whatever Microsoft still supports — csproj TFMs, the `global.json` SDK pin, every workflow's `dotnet-version`, the benchmarks' `RuntimeMoniker`, and the TFM lists quoted in the docs, which is why those lists are worth keeping written the way they already are. A TFM added or dropped by hand in between will be reconciled by the next run; `#if NET*_OR_GREATER` guards are deliberately left alone, so keep guarding by TFM name rather than by .NET version.
