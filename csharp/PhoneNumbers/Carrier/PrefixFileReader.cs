 ...
 */
    public class PrefixFileReader
    {
        ...
        {
            var source =
                typeof(PhoneNumberUtil).Assembly.GetManifestResourceStream(phonePrefixDataDirectory + "config");
            try
            {
                mappingFileProvider.ReadExternal(source);
            }
            catch (IOException)
            {
            }
        ...
        {
            var source = typeof(PhoneNumberUtil).Assembly.GetManifestResourceStream(phonePrefixDataDirectory + fileName);
            try
            {
                var map = new PhonePrefixMap();
                map.ReadExternal(source);
        ...
        public string GetDescriptionForNumber(PhoneNumber number, string language, string script, string region)
        {
            ...
            var phonePrefix = (countryCallingCode != 1)
                ? countryCallingCode
                : (1000 + (int) (number.NationalNumber / 10000000));
